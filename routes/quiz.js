...
router.get("/fetchallquiznoauthentication/:message", async (req, res) => {
  try {
    const count = await Quiz.countDocuments({ code: req.params.message });

    const numberOfRandomRecords = parseInt(req.query.count) || 10; // Default to 20 if not specified

    const randomIndices = Array.from({ length: numberOfRandomRecords }, () =>
      Math.floor(Math.random() * count)
    );

    const quizs = await Quiz.aggregate([
      { $match: { code: req.params.message } },
      { $sample: { size: numberOfRandomRecords } },
    ]);
...
router.post(
  ...
  [
    ...
    body("option1", "option1 must atleast 3 characters").isLength({ min: 1 }),
    body("option2", "option1 must atleast 3 characters").isLength({ min: 1 }),
    body("option3", "option1 must atleast 3 characters").isLength({ min: 1 }),
    body("option4", "option1 must atleast 3 characters").isLength({ min: 1 }),
    body("answer", "answer must atleast 3 characters").isLength({ min: 1 }),
  ],
  async (req, res) => {
    try {
      const {
